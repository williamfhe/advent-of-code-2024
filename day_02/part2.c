...
bool is_report_safe(const int *report, const int count, const int skiped_index)
{
...
bool check_report_safe_with_bad_level(const int *report, const int count)
{
    ...
    // checked without skiping any level
    if (is_report_safe(report, count, -1))
    {
        return true;
    }

    // check with skip
    for (int skiped_index = 0; skiped_index < count; skiped_index++)
    {
        if (is_report_safe(report, count, skiped_index))
        {
            return true;
        }
    }
...
int main()
{
    ...
    for (int i = 0; i < line_count; i++)
    {
        ...
        if (check_report_safe_with_bad_level((const int *)parsed_line.values, (const int)parsed_line.count))
        {
            total_safe++;
        }
    }
